...
class TestResource(Resource):
    def get(self, test_id):
        ...
        session = db_session.create_session()
        test = session.query(Test).get(test_id)
        return jsonify({'test': test.to_dict(only=('type', 'creator', 'title', 'description', 'cover', 'status',
                                                   'user.name'))})
...
class TestListResource(Resource):
    def get(self):
        ...
        return jsonify({'tests': [item.to_dict(only=('id', 'type', 'creator', 'title', 'description', 'cover', 'status',
                                                     'user.name')) for item in tests]})
